...
import { forgeLocally, LOCAL_CANDIDATES } from "@/lib/story/local-forge";
...
export function StoryForge({
  ...
  const search = async (kw?: string) => {
    ...
    try {
      ...
      // 没接上 AI 也不能让功能"看起来坏了"：退回本地候选池
      setPosts(LOCAL_CANDIDATES);
      setLocalMode(true);
    } finally {
      setSearching(false);
    }
  ...
  const create = async (q: string, a: string, busyKey?: string) => {
    ...
    try {
      const entry = saveGenerated(await generate({ data: { question: q, answer: a } }));
      toast("世界线生成完成", { description: entry.data.hook });
      onCreated(entry.id);
    } catch {
      // 未接入 AI：改用本地改编器，把原文重新编排成互动结构（台词全部取自原文）
      const entry = saveGenerated(forgeLocally(q, a));
      toast("已用本地改编生成", {
        description: "未接入 AI：台词全部取自你投喂的原文，结构由本地编排。",
      });
      onCreated(entry.id);
    } finally {
      setPending(null);
    ...
    >
      ...
      {tab === "pick" ? (
        <div className="mx-auto flex max-w-[560px] flex-col gap-3 px-4 py-4">
          ...
          {localMode && (
            <p className="flex items-start gap-1.5 rounded-xl border border-story-glow/25 bg-story-glow/10 px-3 py-2 text-[12px] leading-relaxed text-story-glow">
              <Sparkles className="mt-0.5 size-3.5 shrink-0" />
              <span>
                本地模式：当前环境未接入 AI，这里展示本地整理的热点话题；生成时用本地改编器把原文重新编排成剧情。
              </span>
            </p>
          )}
      ...
      {(loading || pending) && (
        <div
          className="fixed inset-0 z-[80] flex flex-col items-center justify-center gap-5 bg-story-night/92 px-8 text-center backdrop-blur-xl"
          role="status"
          aria-live="polite"
        >
          {/* 生成要等 20~60 秒，让看山陪着等 */}
          <img
            ...
            width={200}
            height={200}
            className="size-36 object-contain drop-shadow-[0_10px_30px_oklch(0.1_0.02_260/60%)]"
          />
          <p className="text-[17px] font-semibold text-story-ink">看山正在为你画出这一境……</p>
          <p className="max-w-[280px] text-[13px] leading-[1.8] text-story-ink/55">
            正在把这条回答改写成剧情、分支与结局，通常需要 20~60 秒
          </p>
          <span className="mt-1 inline-flex items-center gap-2 text-[12px] text-story-ink/40">
