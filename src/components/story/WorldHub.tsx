            ...
            >
              ...
              >
                ...
                <span className="relative block h-1/2 w-full overflow-hidden">
                  {world.card.coverImage ? (
                    ...
                    >
                      <Icon className="hub-glyph size-14 text-white/85" strokeWidth={1.2} />
                    </span>
                  )}
                  {/* 顶部轻压暗，让徽标在任何封面上都可读 */}
                  <span
                    className="absolute inset-x-0 top-0 h-12 bg-gradient-to-b from-black/40 to-transparent"
                    aria-hidden="true"
                  />
                  {/* 顶部徽标：深色半透明底 + 白字，固定可读不依赖主题 */}
                  <span className="absolute left-2.5 top-2.5 flex flex-col items-start gap-1.5">
                    {rank && (
                      <span
                        className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold tracking-wide backdrop-blur-md ${
                          rank <= 3
                            ? "bg-kanshan-blue/90 text-white"
                      ...
                      >
                        ...
                        No.{rank}
                        {post && <span className="font-normal opacity-85">· {heatLabel(heatScore(post))}</span>}
                      </span>
                    )}
                    {world.generated ? (
                      <span className="inline-flex items-center gap-1 rounded-full bg-black/45 px-2 py-0.5 text-[10px] tracking-widest text-white/90 backdrop-blur-md">
                        <Wand2 className="size-2.5" />
                        我生成的
                      </span>
                    ) : (
                      world.id === currentWorldId && (
                        <span className="rounded-full bg-black/45 px-2 py-0.5 text-[10px] tracking-widest text-white/90 backdrop-blur-md">
                          当前世界
                        </span>
                      )
                    )}
                  </span>
                  <span className="absolute right-2.5 top-2.5 inline-flex items-center gap-1 rounded-full bg-black/45 px-2 py-0.5 text-[10px] text-white/90 backdrop-blur-md">
                ...
                <span className="absolute inset-x-0 bottom-0 flex h-1/2 flex-col gap-1.5 bg-white p-3.5">
                  ...
                  <span className="mt-0.5 flex items-center justify-between gap-1.5 text-[11px] text-hub-ink/55">
                    <span className="flex min-w-0 items-center gap-1">
                      ...
                      <img
                        src={mascotFor(world.id)}
                        alt=""
                        aria-hidden="true"
                        loading="lazy"
                        width={64}
                        height={64}
                        className="size-4 shrink-0 object-contain opacity-90 transition-transform duration-300 group-hover:scale-110"
                      />
                      <span className="truncate">{world.card.players}</span>
                    </span>
                    {world.status === "playable" && (
                      <span className="shrink-0 font-medium text-kanshan-blue">
              ...
              {world.generated && (
                <button
                  ...
                  className="absolute right-2.5 top-9 z-10 grid size-7 place-items-center rounded-full bg-black/45 text-white/80 backdrop-blur-md hover:text-white"
                >
