...
| `ZHIHU_OAUTH_APP_ID` / `ZHIHU_OAUTH_APP_KEY` / `ZHIHU_OAUTH_REDIRECT_URI` | 知乎账号登录（**用户侧只需点授权，不需要任何额外操作**） | 登录入口仍在，但点开会提示缺少哪几项；**其余功能全部正常** |
| `ZHIHU_ACCESS_SECRET` | **仅服务端运维配置**：拉取用户创作/关注数据时鉴权"调用方"。不出现在任何用户界面 | 登录照常可用；个人页的创作/关注列表显示"还在接入中" |
...
| 凭证 | 谁提供 | 用户可见吗 |
|---|---|---|
| `appId` / `appKey` / `redirectUri` | 我们（活动页登记后配置） | 不可见，仅用于服务端拼授权地址与换 token |
| 用户授权（点击确认） | **用户本人** | 用户唯一要做的事 |
| `accessSecret` | 我们（开放平台个人中心） | **绝不出现在用户界面**——它属于服务端运维配置 |

所以产品上：**用户只需要"用知乎账号登录"**，不需要填任何东西。
`accessSecret` 缺失时，登录与门禁照常工作，只有"创作/关注列表"这一块会显示"还在接入中"。
...
- **没有 `accessSecret` 时**：登录正常、门禁正常，个人页的列表区显示"已登录成功 ✓ / 列表还在接入中"，
  不向用户暴露任何技术细节（这是运维配置，不是用户要做的事）。
- 登录所需的三项没配好时，门禁面板会如实列出缺哪几项、去哪里拿，而不是给一个点了没反应的按钮。
